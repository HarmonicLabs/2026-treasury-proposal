...
# fund-proposal.sh - Fund the proposal signing address with enough ADA for the governance deposit.
# Sends ADA from the HW wallet to the proposal key's address using a simple transfer
# that the hardware wallet CAN sign (no governance features).
#
...
# Deposit + margin for fees
DEPOSIT="${GOVERNANCE_ACTION_DEPOSIT:-100000000000}"
FEE_MARGIN=5000000
FUND_AMOUNT=$((DEPOSIT + FEE_MARGIN))

echo "=== Fund Proposal Address ==="
echo ""
echo "Network:          ${NETWORK_FLAG[*]}"
echo "From:             ${PAYMENT_ADDRESS}"
echo "To:               ${PROPOSAL_ADDRESS}"
echo "Amount:           ${FUND_AMOUNT} lovelace (deposit + fee margin)"
echo ""
...
echo ""
echo "Proposal address funded successfully."
