...
- New tagged releases of the four maintained libraries, each carrying support for the upcoming intra-era hard fork (Plutus V4 + revised cost model + new protocol parameters):
  - `cardano-ledger-ts` at https://github.com/HarmonicLabs/cardano-ledger-ts
  - `ouroboros-miniprotocols-ts` at https://github.com/HarmonicLabs/ouroboros-miniprotocols-ts
  - `plutus-machine` at https://github.com/HarmonicLabs/plutus-machine
  - `uplc` at https://github.com/HarmonicLabs/uplc
- Pebble release with the type system finalized (full inference, sum types, generics) and Plutus V4 codegen.
...
- Pebble has a tagged release on https://github.com/HarmonicLabs/pebble within the milestone window.
- The ≥3 example contracts compile to UPLC and execute successfully on a Plutus V4 preview/preprod node (a committed log or link to a successful tx is sufficient).
...
- *Hard fork timing.* The hard fork date is set by Intersect / IOG, not HLabs. If it slips by ≤ 1 quarter, this milestone slips with it without affecting the other three. If it slips by > 1 quarter, HLabs will repurpose the freed time toward bringing forward Milestone 2 deliverables and notify the oversight committee.
...
- A new tagged Pebble release exists with namespaces, tests, and the standard library additions (visible in release notes).
...
- Pebble Language Server Protocol (LSP) implementation: syntax highlighting, autocompletion, go-to-definition, find-references, hover docs, inline diagnostics.
- Pebble VS Code extension published to the Marketplace.
- Pebble CLI watch mode + REPL for interactive development.
...
- The Pebble VS Code extension is **publicly listed on the VS Code Marketplace** with a non-trivial install count or a reachable Marketplace URL.
- The Pebble CLI `pebble build --watch` and `pebble repl` (or equivalent) commands work on a freshly cloned example project, reproducible from a public README walkthrough.
- The four libraries remain on a current release within the window (no drift behind protocol changes).
...
- *VS Code Marketplace publication.* Microsoft's review process is generally fast (days) and well-understood; no material risk expected.

#### Milestone 4 (Q1 2027, Jan–Mar): Documentation, Tutorials & Adoption Push

**Deliverables**

- Complete reference documentation for the Pebble language: every language feature documented with at least one runnable example.
...
- Final-quarter library maintenance pass: ensuring the four maintained libraries are aligned with the protocol state at the end of the proposal period and ready for handover into 2027 maintenance funding (separate proposal).

**Acceptance criteria** (oversight committee verifies)

- The Pebble documentation site (or repo `docs/` tree) covers every language feature, and each documented feature has a runnable example committed to the repo.
- The ≥3 end-to-end tutorials are published and each links to a successful preprod transaction demonstrating the contract working on-chain.
- The four maintained libraries each have a release within the milestone window passing their test suites.
