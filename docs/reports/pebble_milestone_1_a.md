...
The substantive updates are on the Plutus side: a batch of new and expanded builtins across Plutus
V1/V2/V3 plus cheaper execution costs. Supporting the hard fork therefore splits across the repos as follows:

- **uplc / plutus-machine**: new builtins and semantics implementation (the bulk of the work)
...
- **buildooor**: same story, plus pick up the evaluator and cost model changes so ex-unit
...
| `@harmoniclabs/cardano-ledger-ts` | 0.5.6 |
| `@harmoniclabs/buildooor` | 0.2.9 |

---

## uplc + plutus-machine

This is where most of the van Rossem work lives, because PV11 is largely "more Plutus".
The new builtins and UPLC features were added to **uplc** and **plutus-machine**:

- UPLC case expressions for `Bool`, `Integer`, and `Data`
- new builtins: `Array` type and operations, `MaryEraValue` operations, modular
  exponentiation (`expModInteger`), `dropList`, and BLS12-381 multi-scalar multiplication

The machine had to be able to run these so we can compute execution units, and the
...
model has gone from 251 to 297 to 350 entries across the Chang/Plomin updates, and PV11 extends it again.

The important design point: nothing downstream hardcodes the cost model list length, so a
...
are what actually serve the hard fork. The work here was three things.

### 1. Updated support for van Rossem
...
- **No new transaction shape.** The Conway tx / block / governance encoders and decoders handle
  van Rossem transactions unchanged. No changes are needed until the actual Dijkstra era.
...
- **AuxiliaryData decode bugs** (issue #18, PRs #19 and #20 from Max, merged). The default
  ...
  on *every* tag-259 aux data. There was also a `TxMetadata` dual-class problem where the package exported
  one `TxMetadata` identity but `AuxiliaryData` checked `instanceof` against a different one, so
  every metadata/memo build threw.
- **De-duplicated `TxMetadata`** so there is a single class identity, with `src/` as the source of
...
and cross-referenced it with Dingo (gOuroboros), since it is one of the complete nodes running on
the Musashi testnet:

- non-segregated block body (CIP-0176)
- nasty transactions (CIP-118)
- guard scripts, which are native script variant 6 and replace `required_signers`
...
It is groundwork for the next hard fork, done now while the spec is fresh.
...
- cross-checked our work against three independent sources:
...
No source changes were needed for van Rossem. It is an intra-Conway fork with no new tx shape, and
buildooor already builds on `cardano-ledger-ts@^0.5.3` (Conway types plus unbounded protocol version),
...
(`plutus-machine` + `uplc`) to estimate execution units. That is a dependency-currency thing, not a
code change:

- bump `uplc` and `plutus-machine` to the PV11 releases so the builder can evaluate scripts that use
  the new builtins
- bump or use PV11 cost models so ex-unit and fee estimates are correct. Best practice is to feed
...
For later reference, Dijkstra is a real builder effort, not a dep bump like van Rossem:

- **build Dijkstra tx types**: the core change is to construct `DijkstraTxBody`/`DijkstraTx` instead of
...
- **guard scripts and new redeemer tag**: native script variant 6 (`script_require_guard`) and the new
  `6 = guarding` redeemer tag. buildooor only handles 0 to 5 today.
...
- **direct deposits (25) and account balance intervals (26)**: new build-arg surface, if we want to expose them.
- **nasty transactions (CIP-118)**: the largest item. A whole new construction API plus balancing plus
  `required_top_level_guards`.

### In Progress

A separate Dijkstra buildooor branch that evolves as we get closer to the era release/hard fork.
...
low-burden, with no new transaction shape, so cardano-ledger-ts and buildooor needed verification and
dependency currency rather than new wire-format code. We used the opportunity to fix some real
Conway aux-data bugs and to build the Dijkstra era ahead of the next fork.

On cardano-ledger-ts specifically: only the Dijkstra era was added, with barrel exports. None of
